...
interface TodoItemProps {
  ...
  handleUpdate: <T extends { id: number; value: string; isCompleted: boolean }>(
    arg: T
  ) => void;
}
...
const TodoItem: React.FC<TodoItemProps> = ({
    ...
    if (isEdit) {
      return (
        <div>
          ...
          />
          <UpdateBtn data-testid="cancel-button" onClick={handleEdit}>
            취소
          </UpdateBtn>
        </div>
      );
  ...
  return <Row>{renderTodo(isEdit, todoItem.todo)}</Row>;
};
